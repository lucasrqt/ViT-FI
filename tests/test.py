...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=[configs.CPU, configs.GPU_DEVICE],
    )
    parser.add_argument(
        "-M",
        "--microop",
    ...
    parser.add_argument(
        "-s", "--seed", type=int, default=configs.TORCH_SEED, help="Random seed."
    )
    parser.add_argument(
...
def run_injections(model, model_for_fault, images, labels, device) -> None:
    ...
    for j in range(len(images)):
        print(
            f" [+] Image {j+1} - Ground truth: {labels[j]} - Prediction without fault: {out_wo_fault[j].item()} - Prediction with fault: {out_with_fault[j].item()}"
        )
...
def main() -> None:
    ...
    test_set = tv_datasets.imagenet.ImageNet(
        root=configs.IMAGENET_DATASET_DIR, transform=transforms, split="val"
    )
    ...
    microbench_outputs = [
        file for file in os.listdir("data/microbench/") if config in file
    ]
    ...
    for output in microbench_outputs:
        ...
        out_wo_faults, out_w_fault = run_injections(
            model, model_for_fault, images, labels, device
        )
        handler.remove()
            ...
            dfs.append(
                {
                    ...
                    "microop": microop,
                    "precision": precision,
                    "diff_threshold": fault_model_threshold,
                    "file": output,
                    "image": i,
                    "label": labels[i].item(),
                    "prediction_wo_fault": out_wo_faults[i].item(),
                    "prediction_w_fault": out_w_fault[i].item(),
                }
            )
    ...
    for i, row in result_df.iterrows():
        if row["prediction_wo_fault"] != row["prediction_w_fault"]:
            crit_err = True
            print(
                f" [+] File {row['file']} - Image {row['image']} - Ground truth: {row['label']} - Prediction without fault: {row['prediction_wo_fault']} - Prediction with fault: {row['prediction_w_fault']}"
            )
...
def main2() -> None:
    ...
    for layer_id, layer in enumerate(layers):
        ...
        handler = layer.register_forward_hook(hook.hook_fn_to_inject_fault)
        out_wo_fault, out_w_fault = run_injections(
            model, model_for_fault, images, labels, device
        )
        handler.remove()
            ...
            dfs.append(
                {
                    ...
                    "image": i,
                    "label": labels[i].item(),
                    "prediction_wo_fault": out_wo_fault[i].item(),
                    "prediction_w_fault": out_w_fault[i].item(),
                }
            )
    ...
    for i, row in result_df.iterrows():
        if row["prediction_wo_fault"] != row["prediction_w_fault"]:
            crit_err = True
            print(
                f" [+] Layer_id {row['layer_id']} - Image {row['image']} - Ground truth: {row['label']} - Prediction without fault: {row['prediction_wo_fault']} - Prediction with fault: {row['prediction_w_fault']}"
            )
