...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-S", "--shuffle-dataset", default=False, action="store_true", help="Shuffle the dataset or not.")
...
def run_injections(model_name, dataset_name, microop, model, model_for_fault, data_loader, precision, device, batch_size, result_df, result_file, logger) -> None:
    global TIME_MEASURE
    model.eval()
    model.to(device)
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        for j in range(len(images)):
            logger.debug(f"Image {(i*batch_size)+j+1} - Ground truth: {labels[j]} - Prediction without fault: {out_wo_fault[j].item()} - Prediction with fault: {out_with_fault[j].item()}")
...
def get_faulty_top5(model_name, microop, model, data_loader, precision, device, batch_size, logger) -> None:
    model.eval()
    model.to(device)
...
def main() -> None:
    ...
    if inject_on_corr_preds:
        ...
        logger.info(f"Injecting faults on correct predictions only.")
    ...
    if save_top5prob:
        get_faulty_top5(model_name, microop, model_for_fault, data_loader, precision, device, batch_size, logger)
    else:
        run_injections(model_name, dataset_name, microop, model, model_for_fault, data_loader, precision, device, batch_size, result_df, result_file, logger)
