...
DATA_RESULT = "data/time_data.csv"
...
class DummyInjection():
    ...
    def hook_fn_to_inject_fault(self, module, module_input, module_output) -> None:
        ...
        faulty_output *= (1 + 0.05)
        return faulty_output.to(configs.GPU_DEVICE)
...
class GetLayerSize():
    ...
    def hook_fn_to_get_layer_size(self, module, module_input, module_output, name, layer_id) -> None:
        ...
        _HOOKABLE_LAYERS.append((name, layer_id, module, self.microop_size, self.input_size))
...
def hook_microop(model, model_name, microop, batch_size, dummy_input) -> torch.utils.hooks.RemovableHandle:
    handlers = list()
    for layer_id, (name, layer) in enumerate(model.named_modules()):
        if layer.__class__.__name__.strip() == microop:
            ...
            hook = GetLayerSize()
            hook_w_params = partial(hook.hook_fn_to_get_layer_size, name=name, layer_id=layer_id)
...
def main() -> None:
    ...
    for model_name in MODELS:
        ...
        for microop in MICROOPS[model_name]:
            ...
            layers = {
                LayerChoice.FIRST: _HOOKABLE_LAYERS[0][MODULE],
                LayerChoice.LAST: _HOOKABLE_LAYERS[-1][MODULE],
                LayerChoice.MIDDLE: _HOOKABLE_LAYERS[len(_HOOKABLE_LAYERS)//2][MODULE],
            }

            for lc, layer in layers.items():
                handler = layer.register_forward_hook(DummyInjection().hook_fn_to_inject_fault)

                logger.info(f"Profiling {lc}.")

                start = time.time()
                for _ in range(NB_ITER):
                    dl_iter = iter(test_loader)
                    inputs, _ = next(dl_iter)
                    with torch.no_grad():
                        inputs = inputs.to(configs.GPU_DEVICE)
                        model(inputs)
                ...
                data = {
                    ...
                    "microop": microop,
                    "target": lc,
                    "time": (end-start)/NB_ITER,
                }


                time_data.append(data)

    data_df = pd.DataFrame(time_data)
