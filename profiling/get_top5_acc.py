...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model", type=str, help="Model file", default=configs.VIT_BASE_PATCH16_224
    )
    parser.add_argument(
        "--precision", type=str, default=configs.FP32, help="Precision of the model"
    )
    parser.add_argument(
        "--dataset", type=str, default=configs.IMAGENET, help="Dataset to use"
    )
    ...
    parser.add_argument(
        ...
        help="Device to use",
        choices=[configs.GPU_DEVICE, configs.CPU],
    )
...
def main() -> None:
    ...
    with torch.no_grad():
        print(
            f" [INFO] Getting top5 probabilities for {model_name} on {dataset} dataset"
        )
        for i, (data, target) in enumerate(test_loader):
            data, target = data.to(device), target.to(device)
            output = model(data)
            ...
            for k in range(len(target)):
                ...
                tensor = torch.cat(
                    (top5prob.indices[k].unsqueeze(0), top5prob.values[k].unsqueeze(0)),
                    dim=0,
                )
                torch.save(tensor, path)
                images.append(
                    {
                        ...
                        "top_diff": top5prob.values[k][0].item()
                        - top5prob.values[k][1].item(),
                    }
                )
    ...
    print(
        f" [INFO] Top5 probabilities saved in data/full_dataset_top5/{model_name}_{dataset}_{precision}_top5prob_*.pt"
    )
